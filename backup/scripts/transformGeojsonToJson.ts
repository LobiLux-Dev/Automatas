...
import type { BaseEdge, Edge } from "../src/types/edge";
...
const filter = <T extends AllowedGeneric>(
  feature: Feature<T>[],
...
const run = async () => {
  try {
    const file = Bun.file(`./data/UPA.geojson`);

    const { features: data }: GeoJSON = await file.json();
    ...
    const edges: Edge[] = filter<number[]>(data, GeometryType.LineString)
      ...
      .map((edge) => {
        ...
        if (!from)
          throw new Error("Invalid edge data: 'from' vertex not found");
    ...
    await Bun.write("./data/edges.json", JSON.stringify(edges, null, 2));
    await Bun.write("./data/vertices.json", JSON.stringify(vertices, null, 2));
  } catch (e) {
    console.error("An error occurred while processing the geojson file", e);
  }
};
